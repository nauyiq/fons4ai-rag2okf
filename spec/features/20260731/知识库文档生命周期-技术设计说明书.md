...
#### 9.1.4 前端交互一致性设计（CR-015）

**设计方向**：保留 V4 的冷灰/蓝紫 Design Token、信息密度和三主题基线，不进行视觉换肤。页面的识别元素继续是“当前文件 + 真实处理轨迹”，但轨迹必须由任务状态驱动；无真实顺序的设置入口不再使用 `01/02/03` 编号装饰。

**统一视图状态**：

```text
idle -> dirty -> validating -> submitting -> success
                                      \-> fieldError
                                      \-> formError
                                      \-> conflict
```

- `pageLoading/pageError` 只负责首屏数据，不能被保存、上传、测试、解析或发布错误复用。
- 每个动作维护独立 pending/error 状态；只禁用受影响的控件，按钮文案使用“正在保存”“正在发布”等真实动作名称。
- 字段错误紧邻字段；表单错误位于表单操作区；网络错误保留用户输入并提供可执行恢复动作；成功提示在后续编辑时清除。
- `409` 或 CAS 冲突进入 `conflict`，提示刷新当前数据后重试，不能伪装成普通网络错误。
- 表单记录 initial snapshot；未修改时禁用保存，离开脏表单前确认。主题允许本地预览，但取消、保存失败或放弃离开时恢复已保存主题。

**交互基础组件与格式化边界**：

| 能力 | 设计职责 |
| --- | --- |
| `AppDialog` / `AppDrawer` | `role="dialog"`、标题关联、初始焦点、焦点约束、Esc、遮罩关闭策略、关闭后焦点返回和脏数据确认 |
| `FormFeedback` / `FormActions` | 字段、表单、成功、冲突反馈和单动作 pending；`aria-live`/`role="alert"` 使用统一 |
| `EmptyState` | 区分真实空数据、当前页筛选无结果和加载失败，并提供对应动作 |
| `StatusBadge` | 后端枚举到中文状态、语义色和非颜色文本的单一映射；未知枚举使用“未知状态”并保留诊断值到开发日志 |
| `formatters` | 基于 `Intl.DateTimeFormat`、`Intl.RelativeTimeFormat`、`Intl.NumberFormat` 统一时间、相对时间、文件大小和空值；非法时间不得显示为“刚刚更新” |
| `Pagination` | 使用接口 `page/size/total`；知识库和文档搜索在无服务端 keyword 契约时明确标记“筛选当前页”，不请求或暗示全量结果 |

**任务轮询与局部刷新**：

- 文档详情首屏加载与后台刷新分离；后台刷新不替换主内容为 loading 页面。
- 解析、发布、重新分块或重试返回受理结果后立即重新加载任务摘要，并在状态为 `PENDING/RUNNING` 时启动轮询。
- 单次请求完成后才安排下一次刷新；同一文档最多一个在途轮询请求。
- 进入终态、路由文档变化、页面隐藏达到退避条件或组件卸载时停止/清理定时器；回到页面后按当前任务事实恢复。
- 错误操作的“重试”必须重试原动作；仅刷新数据的控件命名为“重新加载”，不得混用。

**表单、上传和敏感输入**：

- 登录/注册提供字段级校验、密码显示切换和明确的提交中状态；注册条款由真实复选框控制，密码规则与确认一致性实时呈现。
- 上传在提交前展示文件名、类型、大小、目标文件夹和解析方式，执行客户端格式/大小预检；错误显示在上传 Drawer 内。现有 HTTP 封装不提供可靠上传进度时只展示“正在上传”，不虚构百分比。
- 模型连接和档案的创建、编辑、Key 替换分别维护 pending/error/success；取消、关闭、保存成功或组件卸载时清空 API Key 响应式变量。模型测试按钮旁展示可能产生费用的提示。
- 知识库、资料和偏好设置使用 dirty/valid 判断、参数范围与依赖说明；模型模板加载失败不阻塞已保存连接和档案的读取与维护。

**应用外壳与信息架构**：

- 账户头像按钮只负责显式切换菜单，不在同一点击中导航；菜单支持点击外部、Esc、键盘和触摸。
- 设置入口使用“个人资料 / 使用偏好 / 模型配置 / 知识库设置”的真实层级；当前路由仅一个选中态。
- 全局搜索、Workspace 切换和未开放页面若无可执行能力则隐藏或明确显示为不可用，不保留类似可点击的假控件。
- 文档处理轨迹按上传、解析、发布事实计算 active/completed/failed；无法获得事实时使用中性状态，不固定高亮第一步。
...
| 前端交互一致性 | Vitest fake timers、组件键盘测试、Playwright 三主题/移动端链路、敏感字段持久化断言 | AC-042～AC-047 |
...
| AC-042 | 统一表单视图状态、局部反馈和单动作 pending | 字段/表单/冲突错误、重复提交、保留输入和成功提示清理测试 |
| AC-043 | `PageResponse` + Pagination + formatters + EmptyState/StatusBadge | 总数/翻页/当前页筛选、非法时间、大小/状态/空值快照测试 |
| AC-044 | 文档详情局部刷新与单飞轮询控制器 | fake timers 覆盖动作后启动、终态停止、请求不重叠、卸载清理和无整页闪烁 |
| AC-045 | AppDialog/AppDrawer 焦点与脏数据契约 | Tab/Esc/焦点返回、取消无副作用、未保存确认和移动端测试 |
| AC-046 | 注册条款、敏感输入临时状态和模型测试费用提示 | 条款门禁、密码/Key 取消与卸载清理、零持久化和费用提示测试 |
| AC-047 | 路由驱动导航与显式账户菜单 | 点击/键盘/触摸菜单、单选中态、失效入口和处理轨迹状态测试 |
...
| R-021 | 页面级错误与动作错误共用状态 | 单次保存失败清空整页或错误远离触发点 | page/action/field 状态分离，组件测试确认输入和主内容保留 |
| R-022 | 轮询重叠或定时器未清理 | 请求风暴、页面闪烁和离开页面后继续访问 | 单飞请求、终态/路由/卸载清理、fake timers 与请求计数断言 |
| R-023 | 共享 Dialog 焦点处理回归 | 键盘用户无法关闭或焦点丢失 | 单一浮层组件、焦点返回、Esc/Tab E2E 和 reduced-motion 回归 |
| R-024 | API Key 或密码残留在前端状态 | 敏感明文被后续操作或调试工具读到 | 取消/成功/卸载主动清空，禁止 local/session storage，自动化扫描与断言 |
| R-025 | 分页搜索被误解为全库搜索 | 用户遗漏未加载数据或误判总量 | 总数只使用 `total`；当前页筛选明确标记；后续全库搜索需独立 API 契约 |
...
| 2026-08-07 | CR-015 | 统一前端表单反馈、分页与格式化、文档任务轮询、Dialog/Drawer 可访问性、敏感输入清理和应用导航交互；不改变后端 API 与数据结构 |
