...
| 创建模型档案 | `POST /model-profiles` | `SaveProfileInput` | `ModelProfile` | 依赖 connectionKey；modelName 级联选择 |
...
#### 3.1.2 新增：只读模型市场目录 API `/model-catalog`

所有用户共享的只读市场模板目录（可缓存），驱动右侧市场渲染与档案级联选择。

| 操作 | 方法与路径 | 参数 | 响应 |
| --- | --- | --- | --- |
| 拉取模型目录 | `GET /model-catalog` | `type?: ModelType`（可选按类型过滤） | `ModelCatalog` |

```typescript
/** 市场目录：提供商模板 + 旗下模型清单 + 类型统计 */
export interface ModelCatalog {
  providers: CatalogProvider[]
  typeCounts: Record<ModelType, number>  // 各类型模型总数（All标签用）
}

export interface CatalogProvider {
  providerCode: string
  providerName: string
  defaultBaseUrl: string           // 新增连接时预填
  officialUrl?: string             // 官方跳转↗链接
  /** 该提供商旗下支持的所有模型（按类型过滤后仍为列表子集） */
  models: CatalogModelItem[]
  /** 支持的类型标签（从 models 去重聚合） */
...
> 缓存策略：前端 localStorage 缓存 30 分钟，key = `rag2okf.modelCatalog`。后端可加 10 分钟本地轻量缓存。
...
| 「展示更多模型▼ 添加」按钮 | 打开档案 Modal 并锁定所属 provider（connectionKey 预填不可改）→ 让用户选择模型类型 → catalog 级联模型名 | 见下节 5.8.3 |

#### 5.8.3 两级 Modal 交互（连接 3 字段表单 → 档案级联表单）
...
**② 模型档案 Modal（级联选择 + 高级参数动态）**：
- 字段：provider 下拉（锁定时禁用不可改）→ modelType 单选（7 选 1 Radio.Group）→ modelName Select（根据 providerCode + modelType 从 catalog.models 级联过滤；CUSTOM 提供商降级为手填 a-input 并提示「请确保名称与厂商官方一致」）
