...
public interface DocumentArtifactStore {
     ...
     *
     * @param scope 原文件所属范围（包含 documentKey，用于构造对象路径）
     ...
     * @param scope 产物所属范围
     * @param type 产物类型
     * @param revisionKey ORIGINAL 类型时为 documentKey，其他类型为 revision 业务 key
     * @param detail 附加信息（ORIGINAL 类型时为原始文件名，其他类型为 null）
     */
    record ArtifactReference(ArtifactScope scope, ArtifactType type, String revisionKey, String detail) {

        public ArtifactReference(ArtifactScope scope, ArtifactType type, String revisionKey) {
            this(scope, type, revisionKey, null);
        }
    }
