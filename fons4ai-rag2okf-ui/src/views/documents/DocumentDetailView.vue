...
<template>
  ...
  <section class="document-page detail-page">
    ...
    <div v-if="showRechunk" class="drawer-backdrop" @click.self="showRechunk = false"><section class="modal-panel danger-modal"><p class="eyebrow">DESTRUCTIVE PROCESSING</p><h2>确认重新分块？</h2><p>将删除当前解析结果所对应的既有分块，并按当前分块策略重新处理。原文件和已发布内容不会被删除。</p><footer><button class="secondary-action" data-test="cancel-rechunk" type="button" @click="showRechunk = false">取消</button><button class="danger-action" data-test="confirm-rechunk" :disabled="submitting" type="button" @click="confirmRechunk">删除旧分块并继续</button></footer></section></div>
  </section>
</template>
